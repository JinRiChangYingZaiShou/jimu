...
uint32_t parse_jimu_logger_manager_config(Json &json_config,
    ...
    json_config.at("logger_level_all").get_to(config_var.logger_level_all);
    json_config.at("write_pool_thread_num")
        .get_to(config_var.write_pool_thread_num);

    auto json_array = json_config.at("logger_array").items();
